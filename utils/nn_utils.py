...
class Regressor:
    ...
    def train_and_infer_map(
        ...
        for curr_pred_horizon in PREDICTION_HORIZONS:
            ...
            grid_search = baseline_utils.get_model(
                self, train_input, train_output, args, curr_pred_horizon
            )
            ...
            Parallel(n_jobs=-2, verbose=5)(
                delayed(self.infer_and_save_traj_map)(
                    ...
                    test_centerlines[
                        i : min(i + args.joblib_batch_size, test_num_tracks)
                    ],
                    test_references[
                        i : min(i + args.joblib_batch_size, test_num_tracks)
                    ],
    ...
    def infer_and_save_traj_map(
        ...
        for i in range(test_num_tracks):
            ...
            for (
                ...
                test_input = test_nt_i_curr_candidate[: args.obs_len, :].reshape(
                    (1, 2 * args.obs_len), order="F"
                )
                ...
                y_pred = train_output[neigh_idx][
                    0, :, :horizon, :
                ]  # num_neighbors x curr_pred_len x 2
                test_input = np.repeat(test_input, repeats=args.n_neigh, axis=0)
            ...
            forecasted_trajectories[test_seq_ids[i]] = curr_forecasted_trajectories
    ...
    def infer_and_save_traj_absolute(
        ...
        for i in range(test_num_tracks):
            ...
            curr_test_input = np.repeat(test_input[i], repeats=args.n_neigh, axis=0)
            neigh_output = train_output[neigh_idx][
                i, :, :horizon, :
            ]  # num_neigh x curr_pred_len x 2
